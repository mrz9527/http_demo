...
upload_cb(struct evhttp_request *req, void *arg)
{
	...
	do {
		...
		if (!pos_boundary_start) break; // 无效的body
		...
		if (!filename_start || filename_start >= content_end) break;
	...
	do {
		...
		if (S_ISDIR(st.st_mode)) {
			...
			if(strcmp(type, "text/plain"))
				...
				evutil_snprintf(content_type, sizeof(content_type), "%s;charset=utf-8", type);
